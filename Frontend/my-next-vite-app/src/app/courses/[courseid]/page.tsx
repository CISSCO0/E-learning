...
export default function CoursePage({ params }: { params: Promise<{ courseid: string }> }) {
  ...
  const [students, setStudents] = useState<any[]>([]); // New state to store students
...
const [userId, setUserId] = useState<string>('');  // Assuming you can retrieve the user ID
    ...
    <button onClick={() => handleEditModule(module)}>Edit</button>
      ...
      <ul>
        {students.map((student) => (
          <li key={student._id}>{student.user_id}</li> // You can display other student details here
        ))}
      </ul>
